...
const TESTIMONIALS = [
  {
    ...
    text: 'Trabalhava 44h por semana e mal pagava o aluguel. Em 3 meses na Aquafeel já igualei meu salário, em 6 meses o dobrei. O treinamento é real, o suporte também.',
    earnings: '$5,200 / mês',
  },
  {
    name: 'Diego Oliveira',
    ...
    text: 'Eu não sabia nada de vendas. Aprendi o script, aprendi a escutar o cliente e em 8 meses já estava treinando outras pessoas. A progressão é rápida se você se dedicar.',
    earnings: '$9,800 / mês',
  },
  {
    name: 'Mariana Costa',
    ...
    text: 'O que me vendeu foi a flexibilidade. Escolho meus horários, trabalho de casa quando preciso. Meus filhos me veem mais — e ganho mais do que ganhava em 8h de escritório.',
    earnings: '$4,700 / mês',
  },
];
...
function computeBreakdown(d: CalcData) {
  ...
  const totalUsedYear =
    workYear + sleepYear + mealYear + showerYear + exerciseYear + spouseYear + kidsYear;
...
const Toggle: React.FC<{ label: string; value: boolean; onChange: (v: boolean) => void }> = ({
  ...
  >
    <span>{label}</span>
    <span
      className={`w-10 h-6 rounded-full relative transition-all ${value ? 'bg-cyan-500' : 'bg-slate-700'}`}
    >
      <span
        className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${value ? 'left-5' : 'left-1'}`}
      />
    </span>
  </button>
);
...
export function RecruitingPage() {
  ...
  const STEPS = [
    {
      ...
      content: (
        <div className="space-y-6">
          <Slider
            label="Horas por dia"
            value={calc.workHoursDay}
            min={1}
            max={16}
            onChange={set('workHoursDay')}
          />
          <Slider
            label="Dias por semana"
            value={calc.workDaysWeek}
            min={1}
            max={7}
            unit=" dias"
            onChange={set('workDaysWeek')}
          />
    ...
    },
    {
      ...
      content: (
        <div className="space-y-6">
          <Slider
            label="Horas de sono por dia"
            value={calc.sleepHoursDay}
            min={4}
            max={12}
            onChange={set('sleepHoursDay')}
          />
    ...
    },
    {
      ...
      content: (
        <div className="space-y-6">
          ...
          />
          <Slider
            label="Tempo por refeição (incluindo deslocamento)"
            value={calc.mealMinutes}
            min={10}
            max={90}
            step={5}
            unit="min"
            onChange={set('mealMinutes')}
          />
          <div className="grid grid-cols-3 gap-3 pt-2">
            {[
              ...
              {
                label: 'Por mês',
                value: `${fmt(((calc.mealsPerDay * calc.mealMinutes) / 60) * 30)}h`,
              },
              {
                label: 'Por ano',
                value: `${fmt(((calc.mealsPerDay * calc.mealMinutes) / 60) * 365)}h`,
              },
            ].map((s) => (
              <div
                key={s.label}
    ...
    },
    {
      ...
      content: (
        <div className="space-y-6">
          <Slider
            label="Banhos por dia"
            value={calc.showersPerDay}
            min={1}
            max={3}
            unit="x"
            onChange={set('showersPerDay')}
          />
          <Slider
            label="Minutos por banho"
            value={calc.showerMinutes}
            min={5}
            max={45}
            step={5}
            unit="min"
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            {[
              {
                label: 'Por mês',
                value: `${fmt(((calc.showersPerDay * calc.showerMinutes) / 60) * 30)}h`,
              },
              {
                label: 'Por ano',
                value: `${fmt(((calc.showersPerDay * calc.showerMinutes) / 60) * 365)}h`,
              },
            ].map((s) => (
              <div
                key={s.label}
    ...
    },
    {
      ...
      content: (
        <div className="space-y-6">
          <Slider
            label="Dias por semana (0 = não faço)"
            value={calc.exerciseDaysWeek}
            min={0}
            max={7}
            unit=" dias"
            onChange={set('exerciseDaysWeek')}
          />
          {calc.exerciseDaysWeek > 0 && (
            <Slider
              label="Horas por sessão"
              value={calc.exerciseHoursSession}
              min={0.5}
              max={3}
              step={0.5}
              onChange={set('exerciseHoursSession')}
            />
          )}
          {calc.exerciseDaysWeek === 0 && (
            <p className="text-slate-500 text-sm text-center italic">
              Sem atividade física contabilizada.
            </p>
          )}
          {calc.exerciseDaysWeek > 0 && (
            <div className="grid grid-cols-2 gap-3 pt-2">
              {[
                {
                  label: 'Por semana',
                  value: `${fmt(calc.exerciseDaysWeek * calc.exerciseHoursSession)}h`,
                },
                {
                  label: 'Por ano',
                  value: `${fmt(calc.exerciseDaysWeek * calc.exerciseHoursSession * 52)}h`,
                },
              ].map((s) => (
                <div
                  key={s.label}
    ...
    },
    {
      ...
      content: (
        <div className="space-y-5">
          <Toggle
            label="Tem esposa / parceiro(a)?"
            value={calc.hasSpouse}
            onChange={set('hasSpouse') as any}
          />
          {calc.hasSpouse && (
            <Slider
              label="Horas por dia juntos"
              value={calc.spouseHoursDay}
              min={0.5}
              max={8}
              step={0.5}
              onChange={set('spouseHoursDay')}
            />
          )}
          <Toggle label="Tem filhos?" value={calc.hasKids} onChange={set('hasKids') as any} />
          {calc.hasKids && (
            <Slider
              label="Horas por dia com filhos"
              value={calc.kidsHoursDay}
              min={0.5}
              max={8}
              step={0.5}
              onChange={set('kidsHoursDay')}
            />
          )}
          {!calc.hasSpouse && !calc.hasKids && (
            <p className="text-slate-500 text-sm text-center italic">
              Sem dependentes contabilizados.
            </p>
          )}
        </div>
      ),
  ...
  const googleCalUrl = () => {
    ...
    const guests = ([MANAGER_EMAIL, analystEmail] as string[])
      .filter(Boolean)
      ...
      <section className="relative z-10 text-center px-6 pt-20 pb-16 max-w-4xl mx-auto">
        ...
        <h1 className="text-4xl md:text-6xl font-black leading-tight mb-6">
          ...
          <span className="text-2xl md:text-4xl text-slate-300">
            Descubra o custo real do seu tempo.
          </span>
        </h1>

        <p className="text-slate-400 text-lg max-w-2xl mx-auto mb-10 leading-relaxed">
          Você está trocando <strong className="text-white">horas irreversíveis da sua vida</strong>{' '}
          por um salário que mal cobre as contas. Nós calculamos. Os números vão te surpreender.
        </p>
        ...
        {calcStep === 0 && (
          <div className="bg-white/5 border border-white/10 rounded-3xl p-8 text-center">
            ...
            <p className="text-slate-400 text-sm mb-8 leading-relaxed">
              Vamos calcular, com base na sua rotina real, quantas horas sobram das 24h do dia
              depois de todas as suas obrigações.
            </p>
            <button
        ...
        {calcStep >= 1 && calcStep <= STEPS.length && (
          ...
          >
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-1">
                {STEPS.map((_, i) => (
                  <div
                    key={i}
                    className={`h-1.5 rounded-full transition-all ${i < calcStep ? 'w-6 bg-cyan-500' : 'w-4 bg-white/10'}`}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-8">
              ...
              >
                {calcStep === STEPS.length ? 'Ver meu resultado 🎯' : 'Próximo'}{' '}
            ...
            >
              ...
              <div className="bg-white/5 border border-white/10 rounded-3xl p-8">
                ...
                <p className="text-slate-400 text-sm mb-6">
                  Total disponível: 8.760 horas/ano (24h × 365 dias)
                </p>
                  ...
                  {[
                    ...
                    ...(calc.hasSpouse
                      ? [
                          {
                            label: '❤️ Parceiro(a)',
                            hours: breakdown.spouse.year,
                            color: 'bg-pink-500',
                          },
                        ]
                      : []),
                    ...(calc.hasKids
                      ? [{ label: '👶 Filhos', hours: breakdown.kids.year, color: 'bg-yellow-500' }]
                      : []),
                  ].map((row) => (
                    <div key={row.label}>
                ...
                {/* Free time reveal */}
                <div
                  className={`mt-8 p-6 rounded-2xl border-2 ${breakdown.freeYear < 500 ? 'border-red-500/50 bg-red-500/10' : 'border-cyan-500/40 bg-cyan-500/10'}`}
                >
                  <div className="flex items-center justify-between flex-wrap gap-4">
                    <div>
                      <p className="text-[10px] uppercase tracking-widest text-slate-400 mb-1">
                        ⏰ Seu tempo livre real
                      </p>
                      <p className="text-4xl font-black text-white">
                        {Math.round(breakdown.freeYear)}
                        <span className="text-lg text-slate-400">h/ano</span>
                      </p>
                      <p className="text-slate-400 text-sm">
                        {fmt(breakdown.freeHoursDay)}h por dia — {fmt(breakdown.freePct, 0)}% da sua
                        vida
                      </p>
                    </div>
                    <div
                      className={`text-5xl ${breakdown.freeYear < 500 ? 'grayscale opacity-50' : ''}`}
                    >
                      {breakdown.freeYear < 200
                        ? '😰'
                        : breakdown.freeYear < 500
                          ? '😟'
                          : breakdown.freeYear < 1000
                            ? '🤔'
                            : '😊'}
                    </div>
                  </div>
                  {breakdown.freeYear < 600 && (
                    <p className="mt-4 text-sm text-red-300 font-bold">
                      ⚠️ Com menos de {Math.round(breakdown.freeYear)}h livres por ano, você está
                      trocando sua vida pelo salário atual. Vale a pena?
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-cyan-500/10 to-blue-600/10 border border-cyan-500/20 rounded-3xl p-8">
                ...
                <p className="text-slate-400 text-sm mb-6">
                  Com {Math.round(breakdown.work.year)}h/ano de trabalho você ganha seu salário
                  atual.
                  <br />
                  Na Aquafeel, com <strong className="text-cyan-400">muito menos horas</strong>:
                </p>
                ...
                <p className="text-slate-300 text-sm leading-relaxed border-l-2 border-cyan-500 pl-4">
                  Com mais tempo livre e mais renda, você pode estar com a família, cuidar de você
                  mesmo, e ainda construir um patrimônio real.
          ...
          {[
            {
              label: 'Mês 1–3',
              range: '$1.500 – 3.000',
              desc: 'Aprendendo + primeiras vendas',
              icon: '🌱',
            },
            ...
            <div
              key={e.label}
              className="bg-white/5 border border-white/10 hover:border-cyan-500/30 rounded-2xl p-5 text-center transition-all"
            >
          ...
          {[
            {
              ...
              body: 'Treinamento completo do zero. O que importa é atitude.',
            },
            {
              icon: <Clock size={20} className="text-emerald-400" />,
              title: 'Horário flexível',
              body: 'Trabalhe quando faz sentido para você. Full-time ou part-time.',
            },
            {
              icon: <Users size={20} className="text-violet-400" />,
              title: 'Construa seu time',
              body: 'Recrute, treine e ganhe em cima das vendas da sua equipe.',
            },
          ].map((f) => (
            <div
              key={f.title}
          ...
          {TESTIMONIALS.map((t) => (
            <div key={t.name} className="bg-white/5 border border-white/10 rounded-2xl p-6">
              <div className="flex items-center gap-3 mb-5">
                <div
                  className={`w-10 h-10 rounded-full bg-gradient-to-br ${t.color} flex items-center justify-center text-white font-black text-sm shrink-0`}
                >
                  {t.avatar}
                </div>
                <div>
        ...
        <div className="bg-gradient-to-br from-white/5 to-white/3 border border-white/10 rounded-3xl p-8 md:p-10">
          {!submitted ? (
            <>
              ...
              <p className="text-center text-slate-600 text-xs mt-5">📍 {OFFICE}</p>
            </>
          ) : (
            <div className="text-center py-8">
              ...
              <p className="text-slate-400 text-sm leading-relaxed mb-6">
                Seu agendamento foi enviado para o Google Calendar e nossa equipe foi notificada via
                WhatsApp.
                <br />
                <br />
                <strong className="text-white">Aguarde o contato do gestor</strong> para confirmar o
                horário.
              </p>
