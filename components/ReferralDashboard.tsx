...
export function ReferralDashboard() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#020617] text-slate-50 flex items-center justify-center">
        <div className="animate-spin w-8 h-8 border-4 border-[#22D3EE] border-t-transparent rounded-full"></div>
      </div>
    );
  }
  ...
  if (!token || !data) {
    return (
      <div className="min-h-screen bg-[#020617] text-slate-50 flex flex-col items-center justify-center p-6">
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-50 font-sans pb-12" style={{ fontFamily: "'Outfit', sans-serif" }}>
      {/* Navigation */}
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              ...
              <p className="text-xl font-bold text-[#22D3EE]">{points.points.toLocaleString()} Points</p>
            </div>
            <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 ${isElite ? 'bg-yellow-500/20 border-yellow-500 text-yellow-500' : 'bg-slate-700 border-[#22D3EE] text-white'}`}>
              <User size={20} />
            </div>
            <button
              onClick={() => navigate('/')}
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-8">
        {/* Welcome Hero */}
        <section className={`p-10 rounded-3xl shadow-2xl relative overflow-hidden bg-gradient-to-br ${isElite ? 'from-yellow-600 to-amber-900' : 'from-[#005088] to-[#22D3EE]'}`}>
        ...
        <div className="flex gap-8 border-b border-slate-800 overflow-x-auto pb-2 hide-scrollbar">
          <button onClick={() => setActiveTab('dashboard')} className={`py-2 px-4 font-semibold transition whitespace-nowrap ${activeTab === 'dashboard' ? 'border-b-2 border-[#22D3EE] text-[#22D3EE]' : 'text-slate-400 hover:text-white'}`}>Dashboard</button>
          <button onClick={() => setActiveTab('cadastro')} className={`py-2 px-4 font-semibold transition whitespace-nowrap ${activeTab === 'cadastro' ? 'border-b-2 border-[#22D3EE] text-[#22D3EE]' : 'text-slate-400 hover:text-white'}`}>Cadastrar Família</button>
          <button onClick={() => setActiveTab('arvore')} className={`py-2 px-4 font-semibold transition whitespace-nowrap ${activeTab === 'arvore' ? 'border-b-2 border-[#22D3EE] text-[#22D3EE]' : 'text-slate-400 hover:text-white'}`}>Minha Rede</button>
          <button onClick={() => setActiveTab('premios')} className={`py-2 px-4 font-semibold transition whitespace-nowrap ${activeTab === 'premios' ? 'border-b-2 border-[#22D3EE] text-[#22D3EE]' : 'text-slate-400 hover:text-white'}`}>Prêmios</button>
        </div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="bg-slate-800/70 backdrop-blur-md border border-white/10 p-6 rounded-2xl">
                ...
                <div className="mt-4 flex gap-1">
                  <div className="h-2 w-full bg-[#22D3EE] rounded-full"></div>
                </div>
              </div>
              <div className="bg-slate-800/70 backdrop-blur-md border border-white/10 p-6 rounded-2xl">
                <p className="text-slate-400 text-sm">VENDAS CONVERTIDAS</p>
                <h3 className="text-3xl font-bold mt-1 text-[#22D3EE]">{points.converted_referrals} Vendas</h3>
              ...
              <div className="bg-slate-800/70 backdrop-blur-md border border-white/10 p-6 rounded-2xl">
                ...
                <div className="mt-2 space-y-2 text-sm">
                  <div className="flex justify-between items-center"><span>Venda Convertida</span> <span className="text-[#22D3EE] font-bold bg-[#22D3EE]/10 px-2 py-0.5 rounded">+900</span></div>
                  <div className="flex justify-between items-center"><span>Nova Indicação</span> <span className="text-[#22D3EE] font-bold bg-[#22D3EE]/10 px-2 py-0.5 rounded">+300</span></div>
                </div>
              </div>
            </div>
                ...
                <div className="space-y-4">
                  {history.map((tx: any) => (
                    <div key={tx.id} className="flex justify-between items-center p-4 bg-slate-800 rounded-xl border border-slate-700">
                      ...
                      <div className={`font-bold text-lg ${tx.points > 0 ? 'text-[#22D3EE]' : 'text-red-400'}`}>
        ...
        {activeTab === 'cadastro' && (
          <div className="bg-slate-800/70 backdrop-blur-md border border-white/10 p-8 rounded-2xl max-w-2xl mx-auto animate-in fade-in duration-500">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-[#22D3EE]/20 rounded-full flex items-center justify-center text-[#22D3EE]">
                <Share2 size={24} />
              </div>
              <div>
            ...
            <form className="space-y-5" onSubmit={handleCreateReferral}>
              <div>
                ...
                <input required type="text" value={newRef.name} onChange={e => setNewRef({...newRef, name: e.target.value})} placeholder="Ex: João e Maria" className="w-full bg-[#020617] border border-slate-700 rounded-lg p-3.5 focus:outline-none focus:border-[#22D3EE] transition-colors" />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wider text-slate-400 mb-1">Telefone de Contato</label>
                <div className="flex items-center bg-[#020617] border border-slate-700 rounded-lg overflow-hidden focus-within:border-[#22D3EE] transition-colors">
              ...
              <div>
                ...
                <input type="email" value={newRef.email} onChange={e => setNewRef({...newRef, email: e.target.value})} placeholder="email@exemplo.com" className="w-full bg-[#020617] border border-slate-700 rounded-lg p-3.5 focus:outline-none focus:border-[#22D3EE] transition-colors" />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wider text-slate-400 mb-1">Endereço Residencial (Opcional)</label>
                <textarea value={newRef.address} onChange={e => setNewRef({...newRef, address: e.target.value})} placeholder="Se souber o endereço completo, coloque aqui..." className="w-full bg-[#020617] border border-slate-700 rounded-lg p-3.5 focus:outline-none focus:border-[#22D3EE] h-24 resize-none transition-colors"></textarea>
              </div>
              <button disabled={isSubmitting} type="submit" className="w-full bg-[#22D3EE] text-slate-900 font-bold py-4 rounded-xl shadow-lg hover:bg-[#1fb8d4] transition-colors disabled:opacity-70 disabled:cursor-not-allowed">
        ...
        {activeTab === 'arvore' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-slate-800/70 backdrop-blur-md border border-white/10 p-8 rounded-2xl overflow-hidden">
              ...
              <div className="py-8 bg-[#020617]/50 rounded-2xl border border-slate-700/50">
            ...
            {referrals.length > 0 && (
              <div className="bg-slate-800/70 backdrop-blur-md border border-white/10 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {referrals.map((r: any) => (
                    <div key={r.id} className="bg-[#020617] border border-slate-700 rounded-xl p-4 flex justify-between items-center">
                      <div>
                        <p className="font-bold">{r.name}</p>
                        <p className="text-xs text-slate-400 mt-0.5">{new Date(r.created_at).toLocaleDateString()}</p>
                      </div>
                      <span className={`text-xs font-bold px-3 py-1 rounded-full ${
                        r.status === 'CONVERTED' ? 'bg-[#22D3EE]/20 text-[#22D3EE]' :
        ...
        {activeTab === 'premios' && (
          <div className="animate-in fade-in duration-500">
            <div className="mb-8">
              ...
              <p className="text-slate-400">Troque seus pontos por experiências e produtos incríveis. Seu saldo: <span className="font-bold text-[#22D3EE]">{points.points.toLocaleString()}</span> pts</p>
            </div>
            
            <div className="grid md:grid-cols-3 gap-8">
              {/* Premio 1 */}
              <div className="bg-slate-800/70 backdrop-blur-md border border-white/10 rounded-3xl overflow-hidden group hover:border-[#22D3EE] transition-colors flex flex-col">
                <div className="h-56 bg-[#020617] flex items-center justify-center overflow-hidden">
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <p className="text-[#22D3EE] font-black text-xl mb-4">3.500 <span className="text-sm font-normal text-slate-400 uppercase tracking-widest">pts</span></p>
